-module(helium_region_params_impl).
...
%% ------------------------------------------------------------------
%% helium_gateway_hbvr 'region params' callbacks
%% ------------------------------------------------------------------
...
init(_RPC, StreamState) ->
    ...
    NewStreamState = grpcbox_stream:stream_handler_state(
        StreamState,
        #handler_state{mod = ?MODULE, streaming_initialized = false}
    ),
    NewStreamState.
...
region_params_update(#gateway_region_params_update_req_v1_pb{} = Msg, StreamState) ->
    ...
    #handler_state{streaming_initialized = StreamingInitialized} = grpcbox_stream:stream_handler_state(
        StreamState
    ),
